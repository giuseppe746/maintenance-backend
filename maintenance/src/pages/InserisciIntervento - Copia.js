...
function InserisciIntervento() {
  ...
  useEffect(() => {
    ...
    const savedBg = localStorage.getItem('inserisciColor');
    const savedText = localStorage.getItem('inserisciTextColor');
    if (savedBg) setBgColor(savedBg);
    if (savedText) setTextColor(savedText);

    const logo = localStorage.getItem('logoInserisci');
    if (logo) setLogoInserisci(logo);
